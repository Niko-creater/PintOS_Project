...
void print_linked_list(struct list *l)
{
  ...
  {
    struct blocked_item *s = list_entry(e, struct blocked_item, elem);
    printf("%d\n", s->wake_time); // Assuming 'value' is an integer you want to print
  }
}
...
void timer_sleep(int64_t ticks)
{
  ...
  // thread_block();
  // Create blocked_item
  struct blocked_item *blocked;
  blocked = malloc(sizeof(struct blocked_item));
  if (blocked == NULL)
  {
    // TODO: Decide what has to be done here
    PANIC("couldn't allocate memory for blocked list");
  }
  ...
  print_linked_list(&blocked_threads);
  thread_block();
  // Renable interrupts
  // intr_enable();
  intr_set_level(old_level);
