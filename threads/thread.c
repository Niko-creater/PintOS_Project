...
{
  FPReal factor_1 = FPR_MUL_INT(load_avg, 2);
  FPReal factor_2 = FPR_DIV_FPR(factor_1, FPR_ADD_INT(factor_1, 1));
  return FPR_ADD_INT(FPR_MUL_FPR(factor_2, t->recent_cpu), t->nice);
}
...
FPReal calculate_load_avg(void)
{
  ...
  FPReal factor_1 = INT_DIV_INT(59, 60);
  FPReal factor_2 = INT_DIV_INT(1, 60);
  return FPR_ADD_FPR(FPR_MUL_FPR(factor_1, load_avg), FPR_MUL_INT(factor_2, ready_size));
}
/* Project 2 Advanced Scheduler: Function for updating priority and recent_cpu */
...
void thread_init(void)
{
  ...
  /* Project 2 Advanced Scheduler: Initialize load_avg with 0*/
...
void thread_tick(void)
{
  ...
  if (thread_mlfqs)
  {
    ...
    {
      // Calculation For load_avg
      load_avg = calculate_load_avg();
      // Calculation for recent_cpu
...
void thread_unblock(struct thread *t)
{
  ...
  /* Project 2 Priority Scheduler: 
    After new thread added to ready_list, sort the list so that the first 
    one in the list has the highest priority.
  */
...
void thread_set_priority(int new_priority)
{
  ...
  /* Project 2 Priority Scheduler: 
    yield current_thread if the new priority is not the highest
  */
  ...
  if (!list_empty(&ready_list))
  {
    ...
    struct list_elem *first_pos = list_front(&ready_list);
...
init_thread(struct thread *t, const char *name, int priority)
{
  ...
  /* Project 2 Advanced Scheduler: 
    initialize  nice, recent_cpu and priority of thread
  */
  if (thread_mlfqs) {
    t->nice = 0;
