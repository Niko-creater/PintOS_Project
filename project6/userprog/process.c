...
/* Project5 helper function*/
// Get the child by thread id
struct child *get_child(tid_t tid)
{
  ...
  while (child_elem_ptr != list_end(l))
  {
    ...
    if (child_ptr->tid == tid) // Find the child that belong to the running thread
    {
      return child_ptr;
    }
...
tid_t process_execute(const char *file_name)
{
  ...
  char *save_ptr;
  char *cmd = strtok_r(fn_copy0, " ", &save_ptr); // Get the command

  tid = thread_create(cmd, PRI_DEFAULT, start_process, fn_copy1); // Only use the command as thread name
  palloc_free_page(fn_copy0);
  if (tid == TID_ERROR)
  {
  ...
  // Block the parent thread and waiting for the child
...
start_process(void *file_name_)
{
  ...
  if (success)
  {
    ...
    The argv array first stores the tokens and then store address of each argument on the stack. */
...
int process_wait(tid_t child_tid UNUSED)
{
  ...
  if (child_ptr == NULL)
  {
    return -1; // When thread is not a child of the calling process
  }
